...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 旋转 */
        transform.rotate((GLfloat)timeValue, 0.0f, 0.0f, 1.0f);
        /* 设置‘变换’矩阵的值 */
        shader.setUniformMat4("transformMat", transform.use());
        ...
        transform.reset();
        /* 缩放因子 */
        GLfloat scale = sin(timeValue) / 2 + 0.5f;
        /* 位移 */
        transform.translate(-0.5f, 0.5f, 0.0f);
        /* 缩放 */
        transform.scale(scale, scale, scale);
        /* 设定‘变换’矩阵的值 (使用变换矩阵数组第一个元素地址作为指针传值) */
        shader.setUniformMat4("transformMat", transform.use());
        /* uniform 矩阵变量的 ‘transformMat’ 已经被重新赋值，重新绘制 */
        glDrawElements(GL_TRIANGLES, sizeof(indices)/sizeof(GLuint), GL_UNSIGNED_INT, (void *)0);
