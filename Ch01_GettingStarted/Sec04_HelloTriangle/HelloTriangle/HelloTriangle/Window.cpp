...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    GLuint VBO;     /* 声明引用 顶点缓存对象 ID （Vertex Buffer Object) */
    GLuint EBO;     /* 声明引用 索引缓存对象 ID （Element Buffer Object）*/
    VAO = configVAO(&VBO, vertexBuff, buffLen, &EBO, indices, sizeof(indices));
