...
{
    /* 创建'爆破'着色器 */
    Shader explodeShader(fullPath("ExplodeVS.glsl").c_str(), fullPath("ExplodeGS.glsl").c_str(), fullPath("ExplodeFS.glsl").c_str());
    /* ‘战士’模型加载 */
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        explodeShader.setUniformFloat("timeValue", m_currFTime);
        explodeShader.setUniformMat4("modelMat", modelMat.use());
        /* 用摄像机设置观察矩阵 */
        explodeShader.setUniformMat4("viewMat", g_camera.use());
        /* 设置投影矩阵 */
        explodeShader.setUniformMat4("projectionMat", projectionMat.use());
