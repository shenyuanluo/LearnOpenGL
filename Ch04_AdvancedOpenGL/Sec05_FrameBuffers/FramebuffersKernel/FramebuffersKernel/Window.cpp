...
void Window::render()
{
    ...
    /* 帧缓冲·屏幕四边形着色器 */
    Shader kernelShader(fullPath("FBKernelVS.glsl").c_str(), fullPath("FBKernelFS.glsl").c_str());
    kernelShader.setUniformInt("screenTexture", 0);
...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        if (0 == count % 60)
        {
            kernelShader.setUniformInt("kernelType", type % 4);
            type++;
        }
