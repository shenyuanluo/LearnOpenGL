...
void Window::render()
{
    ...
    /* 帧缓冲·屏幕四边形着色器 */
    Shader screenShader(fullPath("FramebuffersScreenVS.glsl").c_str(), fullPath("FramebuffersScreenFS.glsl").c_str());
    ...
    floorTexture.set2DFilter(GL_REPEAT, GL_REPEAT);
...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 绘制地板 */
        glDisable(GL_CULL_FACE);    // 关闭面剔除，因为地板就一个面，不需要进行面剔除
        modelMat.reset();
        ...
        /* 绘制箱子 */
        glEnable(GL_CULL_FACE); // 设置开启‘面剔除’（默认是关闭的）
        glCullFace(GL_BACK);    // 剔除背面
        glFrontFace(GL_CCW);    // 将逆时针(Counter-Clockwise)定义为正面
        ...
        /* 绑定回默认缓冲并（使用帧缓冲上的纹理附件的颜色缓冲）绘制屏幕四边形 */
        ...
        glBindTexture(GL_TEXTURE_2D, m_fbAttachTexture);    // 再绑定纹理对象到指定的 target：GL_TEXTURE_2D
        glBindVertexArray(quadVAO);
        glDrawArrays(GL_TRIANGLES, 0, 6);
        glBindVertexArray(0);
...
GLuint Window::createFramebuffer(GLuint* attachTexture, GLuint* attachRBO)
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, *attachTexture, 0);   // 将纹理附件添加到帧缓冲上，‘GL_COLOR_ATTACHMENT0’ 作为颜色缓存
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, *attachRBO);   // 将渲染缓冲附件添加到帧缓冲上，‘GL_DEPTH_STENCIL_ATTACHMENT’ 作为深度和模板缓存
