...
void Window::render()
{
    ...
    /* 箱子、地板共用着色器 */
    Shader cubeAndFloorShader(fullPath("StencilTestingVS.glsl").c_str(), fullPath("StencilTestingFS.glsl").c_str());
    /* 箱子·边框着色器 */
    Shader outlineShader(fullPath("StencilTestingVS.glsl").c_str(), fullPath("StencilTestingSingleColorFS.glsl").c_str());
    ...
    /* 设置片段模板值不等于(≠)模板缓冲值时通过模板测试 */
    //    glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
    /* 设置模板、深度测试通过或失败时的行为：
     如果其中的一个测试失败了，什么都不做，仅仅保留当前储存在模板缓冲中的值；
     如果模板测试和深度测试都通过了，将储存的模板值设置为参考值：1（通过 glStencilFunc 设置） */
    glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
    
    /* 设置开启‘深度测试’（默认是关闭的） */
    glEnable(GL_DEPTH_TEST);
    /* 设置片段深度值小于(＜)深度缓冲值时通过深度测试 */
    glDepthFunc(GL_LESS);
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 用摄像机设置观察矩阵 */
        cubeAndFloorShader.setUniformMat4("viewMat", g_camera.use());
        outlineShader.setUniformMat4("viewMat", g_camera.use());
        
        /* 设置投影矩阵 */
        Transform projectionMat;
        projectionMat.perspective(g_camera.fov(), (GLfloat)m_width, (GLfloat)m_height, 0.1, 100.0f);
        cubeAndFloorShader.setUniformMat4("projectionMat", projectionMat.use());
        outlineShader.setUniformMat4("projectionMat", projectionMat.use());
        
        /* 设置模型矩阵 */
        Transform modelMat;
        cubeAndFloorShader.setUniformMat4("modelMat", modelMat.use());
        ...
        /* 开启模板缓冲更新（写入） */
        glStencilMask(0xFF);
        /* 通过使用 GL_ALWAYS 模板测试函数，
         保证了箱子的每个片段都会将模板缓冲的模板值更新为：1 */
        ...
        /* 禁止模板缓冲的写入 */
        glStencilMask(0x00);
        /* 通过使用 GL_NOTEQUAL 模板测试函数，
           保证只绘制箱子上模板值不为 1 的部分(边框) */
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        /* 禁用深度测试 */
        ...
        modelMat.reset();
        modelMat.translate(-1.0f, 0.01f, -1.0f); // 将箱子沿着正 y 轴稍微移动一点，避免深度冲突
        modelMat.scale(1.1f, 1.1f, 1.1f);    // 将箱子放大一点点，给边框留出空间
        outlineShader.setUniformMat4("modelMat", modelMat.use());
        cubeTexture.use2D(GL_TEXTURE0);
        glBindVertexArray(cubeVAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);
