...
void Window::render()
{
    ...
    /* 创建'物体'着色器 对象 */
    Shader depthTestShader(fullPath("DepthTestingVS.glsl").c_str(), fullPath("DepthTestingFS.glsl").c_str());
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 用摄像机设置观察矩阵 */
        depthTestShader.setUniformMat4("viewMat", g_camera.use());
        
        /* 设置投影矩阵 */
        Transform projectionMat;
        projectionMat.perspective(g_camera.fov(), (GLfloat)m_width, (GLfloat)m_height, 0.1, 100.0f);
        depthTestShader.setUniformMat4("projectionMat", projectionMat.use());
        ...
        depthTestShader.setUniformMat4("modelMat", modelMat.use());
