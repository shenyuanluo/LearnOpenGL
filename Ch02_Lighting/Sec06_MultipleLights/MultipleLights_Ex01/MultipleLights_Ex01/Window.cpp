...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    /* ================ 设置光照属性 ================ */
    /* 定向光 */
    DirectionLight dirLight(objectShader, "dirLight");
    /* 点光源 1 */
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 设置清除缓存时窗体背景颜色 */
        glClearColor(scene.bgClearColor(sType).r, scene.bgClearColor(sType).g, scene.bgClearColor(sType).b, scene.bgClearColor(sType).a);
        /* 开始清除（颜色和深度缓冲） */
        ...
        /* ================ 设置光照属性 ================ */
        /* 定向光 */
        dirLight.setDirection(scene.dirLightDirection(sType));
        dirLight.setAmbient(scene.dirLightAmbient(sType));
        dirLight.setDiffuse(scene.dirLightDiffuse(sType));
        dirLight.setSpecular(scene.dirLightSpecular(sType));
        
        /* 点光源 1 */
        pointLight1.setPosition(scene.pointLightPosition(sType)[0]);
        pointLight1.setConstant(scene.pointLightConstant(sType)[0]);
        pointLight1.setLinear(scene.pointLightLinear(sType)[0]);
        pointLight1.setQuadratic(scene.pointLightQuadratic(sType)[0]);
        pointLight1.setAmbient(scene.pointLightColor(sType)[0].x * 0.1,  scene.pointLightColor(sType)[0].y * 0.1,  scene.pointLightColor(sType)[0].z * 0.1);
        pointLight1.setDiffuse(scene.pointLightColor(sType)[0].x,  scene.pointLightColor(sType)[0].y,  scene.pointLightColor(sType)[0].z);
        pointLight1.setSpecular(scene.pointLightColor(sType)[0].x,  scene.pointLightColor(sType)[0].y,  scene.pointLightColor(sType)[0].z);
        
        /* 点光源 2 */
        pointLight2.setPosition(scene.pointLightPosition(sType)[1]);
        pointLight2.setConstant(scene.pointLightConstant(sType)[1]);
        pointLight2.setLinear(scene.pointLightLinear(sType)[1]);
        pointLight2.setQuadratic(scene.pointLightQuadratic(sType)[1]);
        pointLight2.setAmbient(scene.pointLightColor(sType)[1].x * 0.1,  scene.pointLightColor(sType)[1].y * 0.1,  scene.pointLightColor(sType)[1].z * 0.1);
        pointLight2.setDiffuse(scene.pointLightColor(sType)[1].x,  scene.pointLightColor(sType)[1].y,  scene.pointLightColor(sType)[1].z);
        pointLight2.setSpecular(scene.pointLightColor(sType)[1].x,  scene.pointLightColor(sType)[1].y,  scene.pointLightColor(sType)[1].z);
        
        /* 点光源 3 */
        pointLight3.setPosition(scene.pointLightPosition(sType)[2]);
        pointLight3.setConstant(scene.pointLightConstant(sType)[2]);
        pointLight3.setLinear(scene.pointLightLinear(sType)[2]);
        pointLight3.setQuadratic(scene.pointLightQuadratic(sType)[2]);
        pointLight3.setAmbient(scene.pointLightColor(sType)[2].x * 0.1,  scene.pointLightColor(sType)[2].y * 0.1,  scene.pointLightColor(sType)[2].z * 0.1);
        pointLight3.setDiffuse(scene.pointLightColor(sType)[2].x,  scene.pointLightColor(sType)[2].y,  scene.pointLightColor(sType)[2].z);
        pointLight3.setSpecular(scene.pointLightColor(sType)[2].x,  scene.pointLightColor(sType)[2].y,  scene.pointLightColor(sType)[2].z);
        
        /* 点光源 4 */
        pointLight4.setPosition(scene.pointLightPosition(sType)[3]);
        pointLight4.setConstant(scene.pointLightConstant(sType)[3]);
        pointLight4.setLinear(scene.pointLightLinear(sType)[3]);
        pointLight4.setQuadratic(scene.pointLightQuadratic(sType)[3]);
        pointLight4.setAmbient(scene.pointLightColor(sType)[3].x * 0.1,  scene.pointLightColor(sType)[3].y * 0.1,  scene.pointLightColor(sType)[3].z * 0.1);
        pointLight4.setDiffuse(scene.pointLightColor(sType)[3].x,  scene.pointLightColor(sType)[3].y,  scene.pointLightColor(sType)[3].z);
        pointLight4.setSpecular(scene.pointLightColor(sType)[3].x,  scene.pointLightColor(sType)[3].y,  scene.pointLightColor(sType)[3].z);
        
        /* 聚光源 */
        spotlight.setPosition(g_camera.position());
        spotlight.setDirection(g_camera.front());
        spotlight.setCutOff(scene.spotLightCutOff(sType));
        spotlight.setOuterCutOff(scene.spotLightOuterCutOff(sType));
        spotlight.setConstant(scene.spotLightConstant(sType));
        spotlight.setLinear(scene.spotLightLinear(sType));
        spotlight.setQuadratic(scene.spotLightQuadratic(sType));
        spotlight.setAmbient(scene.spotLightAmbient(sType));
        spotlight.setDiffuse(scene.spotLightDiffuse(sType));
        spotlight.setSpecular(scene.spotLightSpecular(sType));
        ...
        for(GLuint i = 0; i < 10; i++)
        {
            ...
            modelMat.rotate(modelMat.radianWithDegree(angle), glm::vec3(1.0f, 0.3f, 0.5f));
        ...
        for (GLint j = 0; j < 4; j++)
        {
            /* 设置模型矩阵 */
            Transform modelMat;
            modelMat.translate(scene.pointLightPosition(sType)[j]);
            modelMat.scale(0.2f, 0.2f, 0.2f); // a smaller cube
            lightShader.setUniformMat4("modelMat", modelMat.use());
            lightShader.setUniformVec3("lightColor", scene.pointLightColor(sType)[j].r, scene.pointLightColor(sType)[j].g, scene.pointLightColor(sType)[j].b);
