...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    /* 反射光纹理 */
    Texture emissionTexture(fullPath("../../../../Resources/TextureImage/matrix.jpg").c_str());
    ...
    /* 设置'放射光纹理‘在纹理单元：2 */
    objectShader.setUniformInt("material.emission", 2);
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 激活，并绑定使用‘镜面光光‘纹理在单元：1 */
        specularTexture.use2D(GL_TEXTURE1);
        
        /* 激活，并绑定使用’放射光‘纹理在单元：2 */
        emissionTexture.use2D(GL_TEXTURE2);
        
        /* 变换 */
        /* 设置模型矩阵 */
        Transform modelMat;
        modelMat.rotate(sin(m_currFTime), cos(m_currFTime), sin(m_currFTime), 0.0f);
