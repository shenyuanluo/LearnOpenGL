...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    /* 环境光照·强度 */
    objectShader.setUniformVec3("light.ambient",  1.0f, 1.0f, 1.0f);
    /* 漫反射光照·强度（将光照调暗了一些以搭配场景） */
    objectShader.setUniformVec3("light.diffuse",  1.0f, 1.0f, 1.0f);
    /* 镜面光照·强度 */
    ...
    Materials material;
    /* 材质颜色·环境光照 */
    objectShader.setUniformVec3("material.ambient", material.info(Material_emerald)[0] , material.info(Material_emerald)[1], material.info(Material_emerald)[2]);
    /* 材质颜色·漫反射光照 */
    objectShader.setUniformVec3("material.diffuse", material.info(Material_emerald)[3] , material.info(Material_emerald)[4], material.info(Material_emerald)[5]);
    /* 材质颜色·镜面光照 */
    objectShader.setUniformVec3("material.specular", material.info(Material_emerald)[6] , material.info(Material_emerald)[7], material.info(Material_emerald)[8]);
    /* 反光度 */
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        {
            y++;
            if (Material_yellowRubber == (MaterialType)(y - 1))
            {
                y = Material_emerald;
            }
            mType = (MaterialType)y;
            Materials material;

            /* 材质颜色·环境光照 */
            objectShader.setUniformVec3("material.ambient", material.info(mType)[0] , material.info(mType)[1], material.info(mType)[2]);
            /* 材质颜色·漫反射光照 */
            objectShader.setUniformVec3("material.diffuse", material.info(mType)[3] , material.info(mType)[4], material.info(mType)[5]);
            /* 材质颜色·镜面光照 */
            objectShader.setUniformVec3("material.specular", material.info(mType)[6] , material.info(mType)[7], material.info(mType)[8]);
            /* 反光度 */
            objectShader.setUniformFloat("material.shininess", 32.0f);
        }
        ...
        /* 设置模型矩阵 */
        Transform modelMat;
        modelMat.rotate(-0.45f, 0.5f, -0.5f, 0.0f);
