...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    /* ================ 设置光照属性 ================ */
    /* 光源位置 */
    objectShader.setUniformVec3("light.position", g_camera.position());
    /* 光源方向 */
    objectShader.setUniformVec3("light.direction", g_camera.front());
    /* 内切光角·余弦 */
    objectShader.setUniformFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
    /* 外切光角·余弦 */
    objectShader.setUniformFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));
    
    /* 环境光照·强度 */
    objectShader.setUniformVec3("light.ambient",  0.1f, 0.1f, 0.1f);
    /* 漫反射光照·强度 */
    objectShader.setUniformVec3("light.diffuse",  0.8f, 0.8f, 0.8f);
    /* 镜面光照·强度 */
    objectShader.setUniformVec3("light.specular", 1.0f, 1.0f, 1.0f);
    
    /* 光强度衰减公式·常数项 */
    objectShader.setUniformFloat("light.constant", 1.0f);
    /* 光强度衰减公式·一次项 */
    objectShader.setUniformFloat("light.linear", 0.09f);
    /* 光强度衰减公式·二次项 */
    objectShader.setUniformFloat("light.quadratic", 0.032f);
