...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    /* 设定物体颜色 */
    objectShader.setUniformVec3("objectColor", 1.0f, 0.5f, 0.31f);
    /* 设定光源颜色 */
    objectShader.setUniformVec3("lightColor",  1.0f, 1.0f, 1.0f);
    /* 设定观察者位置 */
    objectShader.setUniformVec3("viewerPos", g_camera.position());
    
    /* 设置光照因子 */
    /* 环境因子（强度） */
    objectShader.setUniformFloat("ambientStrenth", 0.1f);
    /* 镜面因子（强度） */
    objectShader.setUniformFloat("specularStrength", 0.5f);
    /* 反光因子（强度）*/
    objectShader.setUniformInt("shininess", 32);
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* =================== 变换光源位置 =================== */
        lightPos.x = sin(m_currFTime) / 2.0f + 0.5f;
        lightPos.y = cos(m_currFTime) / 2.0f + 0.5f;
        /* 时时更新光源位置 */
        objectShader.setUniformVec3("lightPos", lightPos);
        ...
        /* 设置模型矩阵 */
        Transform modelMat;
        modelMat.rotate(0.5f, 0.5f, -0.5f, 0.0f);
