...
void Window::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* =================== 设置光照因子 =================== */
        /* 环境因子（强度） */
        objectShader.setUniformFloat("ambientStrenth", sin(m_currFTime)/2 + 0.5f);
        /* 镜面因子（强度） */
        objectShader.setUniformFloat("specularStrength", cos(m_currFTime)/2 + 0.5f);
        /* 反光因子（强度）*/
        if (true == isAdd)
        {
            shininess++;
        ...
        objectShader.setUniformInt("shininessStrength", shininess);
