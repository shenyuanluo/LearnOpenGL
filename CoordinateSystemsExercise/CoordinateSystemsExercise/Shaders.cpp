...
void Shaders::setTrans(const std::string &name, GLfloat* value) const
{
    // OpenGL 开发通常使用一种内部矩阵布局，叫做列主序(Column-major Ordering)布局。GLM 的默认布局就是列主序，所以并不需要置换矩阵，所有填‘GL_FALSE’
    glUniformMatrix4fv(glGetUniformLocation(this->shaderProgramId, name.c_str()), 1, GL_FALSE, value);
}
