...
private:
    ...
    /**
     ...
     @param wrapFormat 纹理环绕方式：GL_REPEAT、GL_MIRRORED_REPEAT、GL_CLAMP_TO_EDGE、GL_CLAMP_TO_BORDER
     @param imgName 纹理图片名
     @return 0，创建失败；> 0，成功创建的纹理引用对象 ID
     */
    GLuint createTexture(GLint internalformat, GLenum pixelFormat, GLint wrapFormat, const GLchar* imgName);
