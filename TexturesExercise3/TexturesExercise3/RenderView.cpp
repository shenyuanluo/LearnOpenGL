...
#pragma mark -- 创建纹理对象
GLuint RenderView::createTexture(GLint internalformat, GLenum pixelFormat, GLint wrapFormat, const GLchar* imgName)
{
    // 声明一个引用 纹理对象 ID
    GLuint textureId;
    // 生成一个纹理对象
...
void RenderView::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    GLuint texture1 = createTexture(GL_RGB, GL_RGB, GL_CLAMP_TO_EDGE, "container.jpg");
    GLuint texture2 = createTexture(GL_RGB, GL_RGBA, GL_CLAMP_TO_EDGE, "awesomeface.png");
