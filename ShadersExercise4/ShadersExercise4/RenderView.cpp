...
void RenderView::render(GLfloat* vertexBuff, GLuint buffLen)
{
    ...
    while (!glfwWindowShouldClose(this->window))
    {
        ...
        GLfloat yoffset = cos(timeValue) / 2.0f;
        GLfloat rcolor  = sin(timeValue) / 2.0f + 0.5f;
        GLfloat gcolor  = cos(timeValue) / 2.0f + 0.5f;
        GLfloat bcolor  = cos(2 * timeValue)/ 2.0f + 0.5f;

        // 改变位置
        customShader.setFloat("xOffset", xoffset);
        customShader.setFloat("yOffset", yoffset);
        
        // 改变颜色
        customShader.setFloat("rColor", rcolor);
        customShader.setFloat("gColor", gcolor);
        customShader.setFloat("bColor", bcolor);
