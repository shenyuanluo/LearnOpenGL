...
void Window::render()
{
    ...
    /* 创建'物体'着色器 对象 */
    Shader nanosuitShader(fullPath("NanosuitVS.glsl").c_str(), fullPath("NanosuitFS.glsl").c_str());
    
    Model nanosuitModel(fullPath("../../../../Resources/Model/nanosuit/Nanosuit.obj").c_str());
    
    /* ================ 设置光照属性 ================ */
    /* 定向光 */
    DirectionLight dirLight(nanosuitShader, "dirLight");
    ...
    /* 点光源 1 */
    PointLight pointLight1(nanosuitShader, "pointLights[0]");
    ...
    pointLight1.setDiffuse(0.8f, 0.8f, 0.8f);
    pointLight1.setSpecular(1.0f, 1.0f, 1.0f);
    
    /* 点光源 2 */
    PointLight pointLight2(nanosuitShader, "pointLights[1]");
    ...
    /* 设定观察者位置 */
    nanosuitShader.setUniformVec3("viewerPos", g_camera.position());
    ...
    while (!glfwWindowShouldClose(m_window))
    {
        ...
        /* 设置模型矩阵 */
        Transform modelMat;
        modelMat.translate(0.0f, -1.75f, 0.0f);
        modelMat.scale(0.2f, 0.2f, 0.2f);
        nanosuitShader.setUniformMat4("modelMat", modelMat.use());
        
        /* 用摄像机设置观察矩阵 */
        nanosuitShader.setUniformMat4("viewMat", g_camera.use());
        
        /* 设置投影矩阵 */
        Transform projectionMat;
        projectionMat.perspective(g_camera.fov(), (GLfloat)m_width, (GLfloat)m_height, 0.1, 100.0f);
        nanosuitShader.setUniformMat4("projectionMat", projectionMat.use());
