...
//        trans = glm::rotate(trans, -timeValue, glm::vec3(0.0f, 0.0f, 1.0f));
        ...
        // 设定‘变换’矩阵的值
        customShader.setTrans("transform", &trans[0][0]);   // 使用变换矩阵数组第一个元素地址作为指针传值
        // uniform 矩阵变量的 ‘transform’ 已经被重新赋值，重新绘制
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        ...
        trans = glm::mat4();
        // 缩放因子
        GLfloat scale2 = cos(timeValue) / 2 + 0.5f;
        // 平移 v(-0.5f, 0.5f, 0.0f)
        trans = glm::translate(trans, glm::vec3(0.5f, 0.5f, 0.0f));
        // 旋转 v(0.0f, 0.0f, 1.0f) 绕 ‘Z’ 轴旋转
//        trans = glm::rotate(trans, -timeValue, glm::vec3(0.0f, 0.0f, 1.0f));
        // 缩放
        trans = glm::scale(trans, glm::vec3(scale2, scale2, scale2));
        // 设定‘变换’矩阵的值
        customShader.setTrans("transform", &trans[0][0]);   // 使用变换矩阵数组第一个元素地址作为指针传值
        // uniform 矩阵变量的 ‘transform’ 已经被重新赋值，重新绘制
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
